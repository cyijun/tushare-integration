...
class FeishuWebHookReporter(Reporter):
    ...
    def send_report(self, subject: str, content: str, *args, **kwargs):
        ...
        if body['content']['post']['zh_cn']['content'][-1][-1]['text'] == '\n':
            body['content']['post']['zh_cn']['content'][-1][-1]['text'] = body['content']['post']['zh_cn']['content'][                -1            ][-1]['text'][:-1]
...
class ReporterLoader(object):
    ALLOWED_REPORTERS = ['tushare_integration.reporters.FeishuWebHookReporter', 'tushare_integration.reporters.DingTalkWebHookReporter']
